...
public class ShipmentREST {
    ...
    WebTarget target = client.target(UriBuilder.fromUri("http://s40server.csse.rose-hulman.edu:9200/s40/shipment").build());
	@GET
	...
	public String updateShipment(@PathParam("id") String id, String json){
		if (!isSame(id, json)){
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public String searchShipment(String json){
	...
	@PUT @Path("{id}")
	@Consumes(MediaType.APPLICATION_JSON)
	public String addShipment(@PathParam("id") String id, String json){
	...
	@DELETE @Path("{id}")
	public String deleteShipment(@PathParam("id") String id){
